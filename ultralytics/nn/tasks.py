...
# djy #
from ultralytics.nn.core11.efficientnet import MBConvBlock,stem


from ultralytics.nn.modules.WTConv import DSConvWithWT,C3k2_WT
from ultralytics.nn.modules.SHSA import SHSA_BLOCK, C2PSA_SHSA
# from ultralytics.change_model.EVSSM import C3k2_SSM_CONV, C2PSA_EVS #失败
from ultralytics.change_model.MixStructure import C3k2_MixStructureBlock
from ultralytics.change_model.FFCM import Fused_Fourier_Conv_Mixer
from ultralytics.nn.modules.LKA_Attention import C2PSA_LKA
from ultralytics.change_model.RHDWT import RHDWT_Block
from ultralytics.change_model.LCA import LCA #失败
from ultralytics.change_model.MFMSAttentionBlock import C2PSA_MFCA
from ultralytics.change_model.LRSA import C2PSA_LRSA
from ultralytics.change_model.CondensedAttention import C2PSA_CAttention
from ultralytics.change_model.TKSA import C3k2_TKSA
...
# swin_transformer
from ultralytics.nn.modules import PatchEmbed,PatchMerging,SwinStage
# efficientnet
from ultralytics.nn.modules import stem, MBConvBlock
# mobilenet
from ultralytics.nn.modules import MobileNetV4ConvSmall,MobileNetV4ConvMedium,MobileNetV4ConvLarge,MobileNetV4HybridMedium,MobileNetV4HybridLarge
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, AsDDet)):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            s = 256  # 2x min stride
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # efficientnet
            ...
            c2 = ch[f[-1]]
        elif m is Fused_Fourier_Conv_Mixer:
            args = [ch[f]]
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect, AsDDet}
        ):
            args.append([ch[x] for x in f])
            if m is Segment or m is YOLOESegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, AsDDet}:
                m.legacy = legacy
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment)):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, YOLOEDetect, v10Detect, AsDDet)):
                return "detect"

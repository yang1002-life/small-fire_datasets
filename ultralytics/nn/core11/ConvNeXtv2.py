...
class LayerNorm(nn.Module):
    ...
    while channels_first corresponds to inputs with shape (batch_size, channels, height,
    width).# https://github.com/iscyy/ultralyticsPro.
    """
...
class ConvNeXtv2(nn.Module):
    ...
    def __init__(self, dim, dim2, drop_path=0.0):
        super().__init__()
...
class ConvNeXtV2(nn.Module):
    ...
    def __init__(
        ...
        cur = 0
        for i in range(4):
            stage = nn.Sequential(*[ConvNeXtv2(dim=dims[i], drop_path=dp_rates[cur + j]) for j in range(depths[i])])

...
class MobileViTB(nn.Module):
    def __init__(self, ch_in, out_in, dim=64, depth=2, kernel_size=3, patch_size=(2, 2), mlp_dim=64 * 2, dropout=0.0):
        super().__init__()  # mg
    ...
    def forward(self, x):
        ...
        x = rearrange(
            x, "b (ph pw) (h w) d -> b d (h ph) (w pw)", h=h // self.ph, w=w // self.pw, ph=self.ph, pw=self.pw
        )
...
def conv_1x1_bn(ch_in, ch_out):
    return nn.Sequential(nn.Conv2d(ch_in, ch_out, 1, 1, 0, bias=False), nn.BatchNorm2d(ch_out), nn.SiLU())


def conv_nxn_bn(ch_in, ch_out, kernal_size=3, stride=1):
    return nn.Sequential(
        nn.Conv2d(ch_in, ch_out, kernal_size, stride, 1, bias=False), nn.BatchNorm2d(ch_out), nn.SiLU()
    )

...
def conv_bn_relu_ori(in_channels, out_channels, kernel_size, stride, padding, groups, dilation=1):
    if padding is None:
...
class ConvGroupShift(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel, stride=1, group=1, bn=True, weight=None):
        super().__init__()
    ...
    def reloc_weight(self, w):
        if w is None:
            return
        _c1, c2, k1, k2 = self.split_convs.weight.data.shape
    ...
    def forward(self, inputs):
        ...
        if self.data is None:
            ...
            *_, ori_h, ori_w = inputs.shape
            ...
            self.data = (c1, c, H, W, h, w, t, isH)
        else:
            c1, c, H, W, h, w, t, isH = self.data
...
class ReparamLargeKernelBlock(nn.Module):
    r"""SLaK_reg ReparamLargeKernelBlock. There are two equivalent implementations: (1) DwConv -> LayerNorm
    (channels_first) -> 1x1 Conv -> GELU -> 1x1 Conv; all in (N, C, H, W) (2) DwConv -> Permute to (N, H, W, C);
    LayerNorm (channels_last) -> Linear -> GELU -> Linear; Permute back We use (2) as we find it slightly faster
    in PyTorch.

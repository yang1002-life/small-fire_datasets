...
def trunc_normal_(tensor, mean=0.0, std=1.0, a=-2.0, b=2.0):
    ...
    :math:`a \leq \text{mean} \leq b`. NOTE: this impl is similar to the PyTorch trunc_normal_, the bounds [a, b]
    are applied while sampling the normal with mean/std applied, therefore a, b args should be adjusted to match the
    range of mean, std args.
...
def _get_rotation_matrix(thetas):
    ...
    rot_mat_positive = torch.cat(
        (
            ...
            torch.cat(
                (
                    torch.zeros(1, 1, bs * g, device=device),
                    x - b,
                    b,
                    torch.zeros(1, 1, bs * g, device=device),
                    1 - c + b,
                    y - b,
                    torch.zeros(1, 3, bs * g, device=device),
                ),
                dim=1,
            ),
            torch.cat(
                (
                    torch.zeros(1, 2, bs * g, device=device),
                    a,
                    torch.zeros(1, 2, bs * g, device=device),
                    1 - a,
                    torch.zeros(1, 3, bs * g, device=device),
            ...
            torch.cat(
                (
                    ...
                    x - b,
                    1 - c + b,
                    torch.zeros(1, 4, bs * g, device=device),
                ),
                dim=1,
            ),
            torch.cat(
                (
                    torch.zeros(1, 4, bs * g, device=device),
                    torch.ones(1, 1, bs * g, device=device),
                    torch.zeros(1, 4, bs * g, device=device),
                ),
                dim=1,
            ),
    ...
    rot_mat_negative = torch.cat(
        (
            torch.cat(
                (c, torch.zeros(1, 2, bs * g, device=device), 1 - c, torch.zeros(1, 5, bs * g, device=device)), dim=1
            ),
            torch.cat(
                (
                    -b,
            ...
            torch.cat(
                (torch.zeros(1, 1, bs * g, device=device), 1 - c, c, torch.zeros(1, 6, bs * g, device=device)), dim=1
            ),
            torch.cat(
                (
                    torch.zeros(1, 3, bs * g, device=device),
            ...
            torch.cat(
                (torch.zeros(1, 6, bs * g, device=device), c, 1 - c, torch.zeros(1, 1, bs * g, device=device)), dim=1
            ),
            torch.cat(
                (
                    torch.zeros(1, 4, bs * g, device=device),
            ...
            torch.cat(
                (torch.zeros(1, 5, bs * g, device=device), 1 - c, torch.zeros(1, 2, bs * g, device=device), c), dim=1
            ),
        ),
        dim=0,
    )  # shape = [k^2, k^2, bs*g]
...
def batch_rotate_multiweight(weights, lambdas, thetas):
    """Let batch_size = b kernel_number = n kernel_size = 3.
...
class ARConv(nn.Module):
    def __init__(
        ...
        kernel_number=1,
        rounting_func=None,
        rotate_func=batch_rotate_multiweight,
    ):
        super().__init__()
        self.kernel_number = kernel_number
    ...
    def extra_repr(self):
        s = (
            "{in_channels}, {out_channels}, kernel_number={kernel_number}"

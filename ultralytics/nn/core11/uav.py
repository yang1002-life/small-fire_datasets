...
class ImprovedFFTKernel(nn.Module):
    def __init__(self, dim) -> None:
        ...
        self.channel_attention = nn.Sequential(
            nn.Conv2d(dim, dim // 4, kernel_size=1), nn.ReLU(), nn.Conv2d(dim // 4, dim, kernel_size=1), nn.Sigmoid()
        )
...
class SemanticAlignmenCalibration(nn.Module):  # 新的模块名称，更具描述性
    ...
    def forward(self, x):
        ...
        semantic_features = F.interpolate(
            semantic_features, coarse_features.size()[2:], mode="bilinear", align_corners=True
        )
        ...
        offset_high = conv_results[:, self.groups * 2 : self.groups * 4, :, :].reshape(
            batch_size * self.groups, -1, out_h, out_w
        )
        ...
        base_grid = (
            base_grid.repeat(batch_size * self.groups, 1, 1, 1).type_as(fused_features).to(fused_features.device)
        )
        ...
        final_features = (
            fused_features * attention_weights[:, 0:1, :, :] + coarse_features * attention_weights[:, 1:2, :, :]
        )

...
class SE(nn.Module):
    def __init__(self, num_out, ratio=4):
        ...
        self.conv_reduce = nn.Conv2d(
            in_channels=num_out, out_channels=num_mid, kernel_size=1, bias=True, padding_mode="zeros"
        )
        self.act1 = Activation("relu")
        self.conv_expand = nn.Conv2d(
            in_channels=num_mid, out_channels=num_out, kernel_size=1, bias=True, padding_mode="zeros"
        )
...
class ConvUnit(nn.Module):
    def __init__(
        self, num_in, num_out, kernel_size=1, stride=1, padding=0, num_groups=1, use_act=True, act_type="relu"
    ):
        super().__init__()
        self.conv = nn.Conv2d(
            in_channels=num_in,
...
class GhostModule(nn.Module):
    ...
    def __init__(
        self, num_in, num_out, kernel_size=1, stride=1, padding=0, ratio=2, dw_size=3, use_act=True, act_type="relu"
    ):
        super().__init__()
...
class GhostModuleMul(nn.Module):
    def __init__(
        ...
        self.short_conv = nn.Sequential(
            ...
            ConvUnit(num_out, num_out, kernel_size=(1, 5), stride=1, padding=(0, 2), num_groups=num_out, use_act=False),
            ConvUnit(num_out, num_out, kernel_size=(5, 1), stride=1, padding=(2, 0), num_groups=num_out, use_act=False),
        )
...
class Ghostblockv2(nn.Module):
    def __init__(self, num_in, num_mid, num_out, kernel_size=3, stride=1, act_type="relu", use_se=False, layer_id=None):
        ...
        if self.use_ori_module:
            self.ghost1 = GhostModule(num_in, num_mid, kernel_size=1, stride=1, padding=0, act_type=act_type)
        else:
            self.ghost1 = GhostModuleMul(num_in, num_mid, kernel_size=1, stride=1, padding=0, act_type=act_type)
        ...
        self.ghost2 = GhostModule(
            num_mid, num_out, kernel_size=1, stride=1, padding=0, act_type=act_type, use_act=False
        )
        ...
        if self.down_sample:
            self.shortcut = nn.Sequential(
                ...
                ConvUnit(num_in, num_out, kernel_size=1, stride=1, padding=0, num_groups=1, use_act=False),
            )

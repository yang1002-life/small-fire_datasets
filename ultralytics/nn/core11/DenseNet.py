...
class DenseLayer(nn.Module):
    def __init__(self, int_numss, gr, bs, norm_layer=BatchNormAct2d, drop_rate=0.0, memory_efficient=False):
        super().__init__()
        (self.add_module("norm1", norm_layer(int_numss)),)
        (self.add_module("conv1", nn.Conv2d(int_numss, bs * gr, kernel_size=1, stride=1, bias=False)),)
        (self.add_module("norm2", norm_layer(bs * gr)),)
        (self.add_module("conv2", nn.Conv2d(bs * gr, gr, kernel_size=3, stride=1, padding=1, bias=False)),)
...
class DenseBlock(nn.ModuleDict):
    ...
    def __init__(self, int_numss, gr, num_layers, bs=4, norm_layer=nn.ReLU, drop_rate=0.0, memory_efficient=False):
        super().__init__()
...
class DenseTrans(nn.Sequential):
    def __init__(self, int_numss, out_numss, kernel_size, norm_layer=nn.BatchNorm2d, aa_layer=None, act=True):
        super().__init__()
        self.conv = nn.Conv2d(int_numss, out_numss, kernel_size=kernel_size, stride=1)
